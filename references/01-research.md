...
1. **Hedef sayfa(lar) ve rol** Â· hedef sayfa + (varsa) aynı markanın rakip-içi sayfaları. Her birinin
   ana sorgudaki organik sırası ve rolü (hub=tanım, listeleme=karşılaştırma, detay=işlemsel).
2. **Ana sorgu SERP'i** Â· `serp_organic_live_advanced` ile **gerçek** ilk sayfa: her sonucun
...
3. **AI Overview / GEO** Â· "<kelime> nedir" ve ana sorgu için AI Overview var mı? Hangi kaynaklar
...
4. **Talep havuzu / content gap** Â· hedef sayfanın/ürünün çevresindeki yüksek hacimli, düşük KD
...
5. **Rakip içerik** Â· ilk sayfadaki editöryel/rakip sayfaların yapısı (nedir tanımı, karşılaştırma
   tablosu, adımlar, geçiş senaryoları). Karşılaştırma matrisi: marka vs rakip içerik öğeleri.
6. **Google Trends** Â· ana sorgunun zaman içindeki ilgisi, zirve/lansman, yükselen sorgular ve
   konular, bölgesel ilgi.
7. **Sosyal duygu** Â· X, Ekşi, Şikayetvar vb. gerçek paylaşımlar/girdiler. Tonu grupla (Olumlu /
   Soru-kafa karışıklığı / Geri bildirim-kapsam). Gerçek kullanıcı adları + özet + öne çıkan nokta.
8. **Sayfa içi & CWV** Â· Title/Meta/H1/H2, Schema (WebPage/Product/FAQPage), görsel alt metin,
   LCP/CLS/INP işaretleri (TTI/DOM proxy), iç bağlantı, onpage skoru. CrUX/PSI ile teyit önerisi notu.
9. **CRO / CX gözlemi** Â· hedef sayfanın hero/CTA, karşılaştırma, değer bloğu, sosyal kanıt,
   mobil öncelik durumu Â· tasarım önerilerinin (03) dayanağı.
